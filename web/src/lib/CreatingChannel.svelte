<script>
  ...
  function onScreenChange(entries) {
    ...
    const { contentRect: { width } } = appElmt 
  ...
  function onUserDetailBoxCreateChan() {
    ...
    const _channelName = selectedUserIds.length <= 1 ? null : username + ", " + channelName 
  ...
  function getUserSearchBoxCls(isSmallScreen) {
    console.log('render')
    let cls = "bg-slate-800 w-[340px] rounded transition-[transform] h-auto duration-200 ease-out border border-slate-800 shadow"
...
<div class=" h-[60px]  flex items-center justify-end">
  ...
  <button class="bg-slate-600 p-1 pl-3 pr-3 rounded-md" on:click={onUserDetailBoxCancel}>x</button>

  {#if renderingSelectedUsers.length }
    <button class="ml-3 p-1 pr-2 pl-2 bg-green-950 rounded-md" on:click={onUserDetailBoxCreateChan}>Create</button>
  {/if}
</div>
  ...
  <div class="flex flex-wrap gap-2 ml-2">
    {#each renderingSelectedUsers as u (u.id)}
      <p class="rounded bg-slate-600 pl-2 pr-2 flex items-center h-[32px]">
        ...
        <button class="ml-3 rounded-[50%] hover:bg-slate-300 w-[16px] h-[16px] flex items-center justify-center"
                on:click={onCancelSelectedUserLiClick(u)}>x
        </button>
      </p>
    {/each}
...
<!-- users search box -->
<ol bind:this={usersBoxElmt}
    class={getUserSearchBoxCls(isSmallScreen)}>
  {#each users as u (u.id)}
    <li class="rounded hover:bg-slate-500 p-3" on:click={() => onUserLiClick(u)}>{u.username}</li>
  {/each}
</ol>
